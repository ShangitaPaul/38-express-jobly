...
describe("POST /companies", function () {
  ...
  test("bad request with missing data", async function () {
    /**
     * Response object returned from the API request.
     * @typedef {import('supertest').Response} Response
     */

    // Make a POST request to create a new company
    const resp /** @type {Response} */ = await request(app)
      .post("/companies")
      .send({
        handle: "new",
        numEmployees: 10,
    ...
    // You can access the response status, headers, and body using the properties of the response object
    // For example, you can check the status code of the response using resp.status
    // You can access the response body using resp.body
    // You can access the response headers using resp.headers
    expect(resp.statusCode).toEqual(400);
  });

  test("bad request with invalid data", async function () {
    /**
     * Response object returned from the API request to create a new company.
    ...
    /**
     * Sends a POST request to create a new company and stores the response in the `resp` variable.
     * @type {ResponseObject}
     */
    const resp = await request(app)
      .post("/companies")
